...
# Definition of the MSE loss: https://pytorch.org/docs/stable/_modules/torch/nn/functional.html

def MSE_loss(predictions, target):
    #return ((predictions-target)**2).sum().item() 
...
def d_MSE_loss(predictions, target):
    ...
    #return 2 * (predictions-target)

def cross_entropy_loss(prediction, target):
    return t.sum(target * t.log(input) + (1-target) * t.log(1-input))

def d_cross_entropy_loss(prediction, target):
...
class Sequential(Module):
    ...
    def display(self):
        ...
        #print(self.output)
        self.monitor_test_loss.append(self.loss(self.output,self.target)) #Monitor the loss
    ...
    def save_params(self, filename):
        ...
        '''
        data = {"test_loss": [tl for tl in self.monitor_test_loss]}
        f = open(filename, 'w')
        json.dump(data, f)
        f.close()
...
network.save_params('loss_hugo.json')
